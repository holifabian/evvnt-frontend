...
import {
  Sparkles, Calendar, Users, DollarSign, FileText, Check, Loader,
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="relative bg-navy rounded-3xl overflow-hidden shadow-xl p-8 sm:p-12 mb-12 text-center text-white">
          ...
          <div className="relative z-10 max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 bg-blue-500/10 border border-blue-400/30 text-blue-300 px-3 py-1 rounded-full text-xs font-bold mb-4">
              <Sparkles size={13} className="animate-pulse" /> PLANIFICADOR INTELIGENTE
            </div>
        ...
        <div className="card p-6 sm:p-8 bg-white border border-gray-100 shadow-sm rounded-2xl mb-12">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label className="label text-navy font-bold">3. Número de invitados</label>
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="menos de 20">Menos de 20 invitados</option>
                    <option value="20-50">De 20 a 50 invitados</option>
                    <option value="50-100">De 50 a 100 invitados</option>
                    <option value="más de 100">Más de 100 invitados</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label className="label text-navy font-bold mb-0">4. Presupuesto total máximo</label>
                <span className="text-2xl font-black text-primary font-mono bg-blue-50 px-3 py-1 rounded-lg">
                  ${presupuesto} USD
                </span>
              </div>
              <input
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full btn-primary py-4 text-base shadow-lg shadow-primary/10 flex items-center justify-center gap-2.5 animate-pulse-ring"
            >
              {loading ? (
                <>
        ...
        {loading && (
          <div className="card p-10 text-center flex flex-col items-center justify-center space-y-4 animate-fade-in bg-white border border-gray-100">
            ...
            <h3 className="text-navy font-black text-lg">Nuestra IA está analizando los mejores proveedores</h3>
            <p className="text-gray-500 text-sm max-w-sm font-medium animate-pulse">{loadingMessage}</p>
          </div>
        )}
        ...
        {resultado && resultado.paquetes && (
          <div id="resultados-ia" className="space-y-10 animate-slide-up">
            <div className="text-center">
              <h2 className="text-2xl font-black text-navy">¡Tus Paquetes Personalizados están listos!</h2>
              <p className="text-gray-500 text-sm mt-1">Compara las opciones generadas por Claude en base a tus requerimientos</p>
            </div>
              ...
              {resultado.paquetes.map((p, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {esRec && (
                      <div className="absolute top-0 right-0 bg-primary text-white text-xs font-black px-4 py-1.5 rounded-bl-xl uppercase tracking-wider flex items-center gap-1 shadow-sm">
                        <Crown size={12} /> RECOMENDADO
                      </div>
                    )}
                    {isPremium && !esRec && (
                      <div className="absolute top-0 right-0 bg-amber-500 text-white text-xs font-black px-4 py-1.5 rounded-bl-xl uppercase tracking-wider flex items-center gap-1">
                        <Award size={12} /> PREMIUM
                      </div>
                    )}
                    {isBasic && !esRec && (
                      <div className="absolute top-0 right-0 bg-slate-500 text-white text-xs font-black px-4 py-1.5 rounded-bl-xl uppercase tracking-wider flex items-center gap-1">
                    ...
                    <div className="p-6 pb-4 border-b border-gray-100">
                      <h3 className="font-black text-navy text-lg pr-20">{p.nombre}</h3>
                      <p className="text-xs text-gray-500 mt-1 font-medium leading-relaxed">{p.descripcion}</p>
                      <div className="mt-4 flex items-baseline gap-1">
                        <span className="text-3xl font-black text-navy font-mono">${p.precio_total}</span>
                    ...
                    <div className="p-6 flex-1 space-y-4">
                      <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest">Servicios incluidos</h4>
                        ...
                        {p.servicios && p.servicios.map((s, sidx) => (
                          <div key={sidx} className="flex gap-2.5">
                            ...
                            <div className="flex-1 text-xs">
                              ...
                              <p className="text-gray-400 text-[10px] font-bold uppercase tracking-wider mt-0.5">{s.categoria}</p>
                    ...
                    <div className="p-6 bg-gray-50 border-t border-gray-100 mt-auto">
                      <button
                        ...
                        className={`w-full py-3 rounded-lg font-bold text-sm flex items-center justify-center gap-2 transition-all ${
                          esRec
                            ? 'bg-primary hover:bg-accent text-white shadow-md shadow-primary/20'
                            : 'bg-white hover:bg-gray-100 text-navy border border-gray-200'
                        }`}
                      >
            ...
            {/* Nota de la IA */}
            {resultado.nota_ia && (
              <div className="card p-5 bg-blue-50 border border-blue-100 rounded-2xl flex items-start gap-4">
                <div className="w-10 h-10 bg-navy rounded-xl flex items-center justify-center shrink-0 shadow-sm">
                  <Sparkles size={18} className="text-white" />
                </div>
                <div>
                  <h4 className="text-navy font-bold text-sm mb-1">Análisis Inteligente (Claude AI)</h4>
